...
function sendMesage(event) 
{
	...
	//if (event.keyCode == 13 && !event.shiftKey) {
		...
		//if (trim(post)) {
			...
			//CHAT.roomJID = "";
			message = {
				message: "hello please work dude",
				name: chatUser.name,
			...
			/*showMessage(qbID,
				...
				//connection.muc.groupchat("8335_group1@muc.chat.quickblox.com", message);
...
function SendMessageCotrain(message, groupID,senderName,senderImg)
{
		message = {
					message: message,
					type:"groupchat",
		...
		connection.muc.groupchat(groupID, message);
		//alert('sent:'+message);
		return false;
}
...
function getRoster(users, room) 
{
	...
	//alert('getRosters:'+usersCount+'\n'+room);
	$('#chat-room .users-count').text(usersCount);
	var output ='';
	for (var entry1 in room) 
	{
		output += 'key1: ' + entry1 + ' | value1: ' + room[entry1] + '\n';
	}
	//alert('output:'+output);
...
function getMessage(stanza, room) 
{
	...
	response1 = checkResponse(response);
	...
	else 
	{
		...
		showMessage(qbID, message, name, roomId, photo, createTime, messageTime, selector);
	}
	
	return true;
}

function showMessage(qbID, message, name, roomId, photo, createTime, messageTime, selector) 
{
	//alert('show message');
	var time = messageTime.substring(11, 19).split(':');
	var hr = time[0];
	var min = time[1];
	var sec = time[2];
	var msgTime =  hr + ": " + min;
	
	owner_QBid = window.sessionStorage.getItem("qbId");
	QBChatRoom = window.sessionStorage.getItem("QBChatRoomName");
	room_id = ""+QBChatRoom+"@muc.chat.quickblox.com";
	//alert('show message:'+owner_QBid+'\n'+qbID);
	
	/*
	var date = new Date(messageTime);
	var hour = date.getHours();
	var minu = date.getMinutes();
	var msgTime =  hour + ": " + minu;
	alert(msgTime);
*/
	if(room_id == roomId)
	{
		...
		if(owner_QBid != qbID)
		{
			var html = '<div class="messageItemLeft"><div class="circleMessage"><img src="'+photo+'" id="user_img"/></div><div><h2>'+name+'</h2><p>'+message+'</p><span>'+msgTime+'</span></div></div><br/>';
		$('#chat').append(html);
		}
		else
		{
			var html = '<div class="messageItemRight"><div class="circleMessage"><img src="'+photo+'"id="user_img"/></div><div><h2>ME</h2><p>'+message+'</p><span>'+msgTime+'</span></div></div><br/>';
